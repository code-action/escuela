        ...
        <div class="col-md-8">
            <div class="card">
              <div class="card-header card-header-info card-header-icon">
                ...
                <h4 class="card-title">{{ __('Lista de Grados') }}</h4>
              </div>
              <div class="card-body">
                <div class="row">
                  <div class="col-12 text-right">
                    <a href="{{ route('grados.create') }}" class="btn btn-sm btn-info">{{ __('Nuevo') }}</a>
                  </div>
                </div>
                    ...
                    <tbody>
                      ...
                      @foreach($grados as $grado)
                        ...
                        <td class="td-actions text-right">
                          <form action="{{ route('grados.destroy', $grado->grd_id) }}" method="post">
                          @csrf
                          @method('delete')
                            <a rel="tooltip" class="btn btn-success btn-link" href="{{ route('grados.edit', $grado->grd_id) }}" title="Editar">
                            <i class="material-icons">edit</i>
                            <div class="ripple-container"></div>
                            </a>
                            <a rel="tooltip" class="btn btn-info btn-link" href="{{ route('grados.show', $grado->grd_id) }}" title="Ver">
                              <i class="material-icons">assignment</i>
                              <div class="ripple-container"></div>
                            </a>
                            @if($grado->alumnos->count()<1)
